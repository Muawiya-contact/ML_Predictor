...
def main():
    """Train the dictionary + Bag-of-Words triage model and save it.

    Guarded because this is destructive: it overwrites the bundle in
    triage_model/ that every predictor loads. All of it used to run at
    module level, so merely importing this module - to read a constant,
    or by any tool that walks the package - silently retrained the model
    and rewrote the deployed artifacts underneath whoever was using them.
    """
