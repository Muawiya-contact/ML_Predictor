...
the working, not only the answer:
  ...
  5. Results            - the serving model's classification metrics.
  6. Cluster Analysis   - translation and similarity across complaints.
...
# A desktop shortcut, a launcher script or an IDE runner
...
class TriageGUI(tk.Tk):
    ...
    def _build_header(self):
        # Reserve room for the title and the status text.
    ...
    def _load_model(self):
        from triage_pipeline import describe_model, load_artifacts
        ...
        # Display the report belonging to the bundle that scores patients.
        report = None
        served_stops = os.path.join(model_dir, "learned_stopwords.json")
        if os.path.exists(served_stops):
            with open(served_stops, "r", encoding="utf-8") as f:
                report = json.load(f)
        return art, report, info, model_dir

    def _done_load_model(self, payload):
        (self.artifacts, self.stopword_report,
         self.model_info, self.model_dir) = payload
    ...
    def active_model_dir(self):
        """Resolve the serving bundle relative to the project."""
    ...
    def _offer_model_pull(self, model="llama3.2"):
        ...
        them. Translation remains unavailable until a model is installed.
        """
        ...
        def poll():
            ...
            if shared["done"]:
                ...
                if shared["ok"]:
                    messagebox.showinfo(
                        "Download complete",
                        f"{model} is installed. Local translation is "
                        f"ready - translation now runs entirely on this "
                        f"machine.")
                    self.status.set(f"{model} installed - translation ready.")
                else:
    ...
    def _do_predict(self):
        ...
        # translate_complaint() has already blocked drift by this point.
        #
    ...
    def _do_explore(self):
        if self.artifacts is None:
            self.status.set("Model is still loading. Try again when it is ready.")
            return
        ...
        en, err = self.translate_complaint(raw, allow_blocked=True)
        if err:
    ...
    def _batch_worker(self):
        ...
        df = read_table(path)
        # Translate the column first and retain each row's gate status.
        # Rows that fail translation or the gate have their scores cleared
        # before the results are displayed or exported.
        texts, failures = [], 0
        ...
        for _i, t in enumerate(df["Complaint_Text"].fillna("").astype(str)):
            ...
            en, err = self.translate_complaint(t, allow_blocked=True)
...
def main():
    ...
    model_dir = resolve_project_file(ENGLISH_MODEL_DIR)
